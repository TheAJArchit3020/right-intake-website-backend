...
exports.generateDietPlanDirectWithTempUser = async (req, res) => {
  ...
  try {
    ...
    console.log(`Fetching temporary user data for tempUserId: ${tempUserId}`);
    const tempUser = await TemporaryUser.findById(tempUserId);
    ...
    let userId;
    if (tempUser.permanentUserId) {
      userId = tempUser.permanentUserId;
    } else {
      const user = new User(tempUser.toObject());
      delete user._id; 
      const savedUser = await user.save();

      tempUser.permanentUserId = savedUser._id;
      await tempUser.save();

      userId = savedUser._id;
    }

    console.log(`Adding diet plan generation job to queue for userId: ${userId}`);
    await dietPlanQueue.add({ userId, email: tempUser.email });

    res.status(200).json({
      message: "Diet plan generation job added to the queue successfully",
      userId,
    });
  } catch (error) {
